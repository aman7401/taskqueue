...
// Store wraps all Postgres operations for the task queue.
type Store struct {
	pool *pgxpool.Pool
}
...
// EnqueueJob inserts a new job into the jobs table.
...
// DequeueJob claims the next available job for the given queue using
// SELECT FOR UPDATE SKIP LOCKED to prevent double-delivery under concurrency.
...
// MarkFailed updates a job after a failed attempt: increments retry_count and
// either reschedules it (pending with backoff) or moves it to the DLQ (dead).
...
// ListJobs returns jobs filtered by optional queue_name and status.
...
// QueueStats returns aggregate counts per status for a queue.
...
// ListDLQ returns dead letter jobs for a queue.
...
// RequeueDLQ moves a dead-letter job back to pending.
func (s *Store) RequeueDLQ(ctx context.Context, dlqID uuid.UUID) (*models.Job, error) {
	...
	const fetchDLQ = `
		SELECT queue_name, payload FROM dead_letter_jobs WHERE id = $1
	`
	...
	const deleteDLQ = `DELETE FROM dead_letter_jobs WHERE id = $1`
	if _, err := tx.Exec(ctx, deleteDLQ, dlqID); err != nil {
		return nil, err
	}
...
// scanJob scans a pgx.Row into a Job.
...
// scanJobRow scans a pgx.Rows (from Query) into a Job.
