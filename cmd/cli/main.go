...
func main() {
	...
	root.AddCommand(submitCmd(), statusCmd(), listCmd(), statsCmd(), dlqCmd())
...
func submitCmd() *cobra.Command {
	...
	cmd.Flags().IntVar(&maxRetries, "retries", 3, "Max retries before DLQ")
	cmd.Flags().StringVar(&runAt, "run-at", "", "Schedule for future time (RFC3339)")
	return cmd
}
...
func dlqCmd() *cobra.Command {
	...
	listDLQ.Flags().StringVarP(&queueName, "queue", "q", "", "Filter by queue")
...
func printJobDetail(m map[string]interface{}) {
	...
	for _, f := range []string{"id", "queue_name", "status", "priority", "retry_count",
		"max_retries", "next_run_at", "started_at", "completed_at", "error_message", "created_at"} {
...
func printJobList(m map[string]interface{}) {
	...
	for _, raw := range jobs {
		...
		fmt.Fprintf(w, "%v\t%v\t%v\t%v/%v\t%v\n",
			...
			j["retry_count"], j["max_retries"], formatTime(j["created_at"]))
	}
	w.Flush()
}
...
func printDLQ(m map[string]interface{}) {
	...
	w := tabwriter.NewWriter(os.Stdout, 0, 0, 2, ' ', 0)
	fmt.Fprintln(w, "DLQ ID\tORIGINAL JOB\tQUEUE\tRETRIES\tERROR\tCREATED")
	for _, raw := range jobs {
		j, _ := raw.(map[string]interface{})
